...
# Check 2: weights added correctly
import numpy as np
rng=np.random.default_rng(42)
for u,v in G.edges(): G[u][v]['weight']=int(rng.choice([-1,1]))
weights=[G[u][v]['weight'] for u,v in list(G.edges())[:10]]
print(f'Check 2 - Weights: first 10 = {weights}')
print(f'  all +-1: {all(w in [-1,1] for w in weights)}')

# Check 3: one AQLS trial completes
