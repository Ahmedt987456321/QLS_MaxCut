"""A/B: plain AQLS vs AQLS + overlap cluster moves on G11."""
...
def run(cluster):
    ...
    for t in range(TRIALS):
        ...
        print(f"  {tag} {t+1}/{TRIALS}: {m.best_cut}")
    return cuts

print("=== plain AQLS ===")
plain = run(False)
print("=== AQLS + cluster moves ===")
clust = run(True)
...
json.dump({"plain": plain, "cluster": clust, "p": p},
          open("results/cluster_vs_plain_G11.json","w"))
